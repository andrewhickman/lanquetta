...
type UpdateQueue = SegQueue<Box<dyn FnOnce(&mut ConnectionController, &mut MethodTabState) + Send>>;
...
impl ConnectionController {
    pub fn new() -> ConnectionController {
...
impl<W> Controller<MethodTabState, W> for ConnectionController
where
...
const UPDATE_STATE: Selector = Selector::new("app.connection.update-state");
...
impl UpdateWriter {
    fn update(
        &self,
        f: impl FnOnce(&mut ConnectionController, &mut MethodTabState) + Send + 'static,
    ) -> bool {
